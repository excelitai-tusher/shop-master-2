                          ...
                          await FacebookAuth.instance
                            ...
                            MaterialPageRoute(
                                builder: (context) => BottomNavBar(
                                    currentTab: 0, currentScreen: HomeScreen(nameFromFacebook: _userObj["picture"]["data"]["url"],routeKey: 1,))),
                                (route) => false));
                      },
                      icon:   Padding(
            ...
            Padding(
              ...
              child: ElevatedButton(
                ...
                onPressed: wait?null:() async{
                  if (numberController.text.isNotEmpty&&_isLoggedIn==false) {
                    int responseStatus =await getPost();

                    if (responseStatus == 200) {
                      Navigator.of(context).push(MaterialPageRoute(
                          builder: (context) =>
                          Otp_screen(mobile: numberController.text.toString(), token: '',from: widget.from,start: widget.start,)
                      )
                      );
                    }
                  } else {
                    if (_isLoggedIn){
                      Navigator.pushAndRemoveUntil(
                          ...
                          MaterialPageRoute(
                              builder: (context) => BottomNavBar(
                                  currentTab: 0, currentScreen: HomeScreen())),
                          (route) => false);
                    }
                  }
                  startTimer();
